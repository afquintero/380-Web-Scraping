...
def recentPosts(IGHandle, postCount = 26):
	...
	try:
		...
		sys.exit("Data cannot be collected from here at this time. The account is private.")
...
def getPostLikes(urls):
	...
	for link in urls:
		...
		try:
			...
			numLikes = postlikes.text
			numLikes = numLikes.replace(',', '')
			likes.append(int(numLikes))
			pic_or_vid = True
...
def getTopPost(likes, picURLs, numOfPosts):

	linksAndLikes = {}

	for i in range(0 , len(likes)):

		linksAndLikes[likes[i]] = str(picURLs[i])
	print(linksAndLikes.values())
    #plzFuckingWork = list(likes)
	mergeSort(likes)
	topPost = likes[len(likes) - 1]


	return linksAndLikes.get(topPost)
		

def retrieveImage(url, filename="insta.jpg"):
	browser = Chrome()
...
igHandle = input("Which Instagram account do you want to scrape? \n")
numOfPosts = input("How many posts should we scrape from this account? (>= 50) \n")

myLinks = recentPosts(igHandle)
#print("Succesfully scraped " + str(len(myLinks)) + " post links from this account:\n " )
#for element in myLinks:
#	print(element + "\n")
postLikes = getPostLikes(myLinks)
browser = Chrome()
browser.get(getTopPost(likes, picURLs, numOfPosts))
